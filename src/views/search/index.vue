<template>
  <div class="search">
    ...
    <van-search show-action placeholder="请输入搜索关键词" clearable v-model.trim="searchInfo">
      <template #action>
        <div @click="goSearch(searchInfo)">搜索</div>
      </template>
    </van-search>
...
export default {
  ...
  methods: {
    goSearch (info) {
      if (info === '') return
      this.searchInfo = ''
      this.history = this.history.filter(item => item !== info)
      this.history.unshift(info)
      setSearchHistory(this.history)
      this.$router.push(`/searchlist?search=${info}`)
    },
    clearHistory () {
      this.history = []
      setSearchHistory([])
